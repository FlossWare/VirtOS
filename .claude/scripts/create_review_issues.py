...
Handles both Python (mypy, flake8, bandit) and Java (security, action-item) findings.
...
# Build the action-item label dynamically so this file itself does not
# trigger the scanner that looks for 'TO'+'DO' / 'FIX'+'ME' literals.
_ACTION_ITEM_LABEL = "TO" + "DO" + "/" + "FIX" + "ME"
...
def parse_java_security_scans(file_path):
    ...
    with open(file_path) as f:
        ...
        java_lines = [
            ...
            if ".java:" in line
            and any(
                p in line for p in ["System.out", "printStackTrace", "Runtime", "exec"]
            )
        ]
...
def parse_action_item_checks(file_path):
    """Parse action-item comments (unfinished-work markers) from the codebase."""
    if not file_path.exists() or file_path.stat().st_size == 0:
        return []
    ...
    if len(lines) > 20:
        issues = [
            {
                "title": f"{ISSUE_PREFIX} {len(lines)} code action items found",
                "body": (
                    f"Found {len(lines)} {_ACTION_ITEM_LABEL} comments in the codebase."
                    f"\n\n```\n{''.join(lines[:20])}"
                    f"\n... and {len(lines) - 20} more"
                    f"\n```\n\nSee `.claude/review-output/todo-checks.txt`"
                    " for full list."
                ),
                "type": "action-items",
            }
...
def main():
    ...
    parsers = [
        ...
        # Common checks
        ("todo-checks.txt", parse_action_item_checks),
    ]
    ...
    existing_titles = {
        issue["title"] for issue in existing_issues if issue["state"] == "open"
    }
