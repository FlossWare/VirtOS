...
export const meta = {
  ...
  phases: [
    { title: 'Sync', detail: 'Fetch and rebase from remote' },
...
if (syncResult.status === 'conflicts') {
  log(`⚠️ Rebase conflicts detected: ${syncResult.conflicts?.join(', ') || 'unknown'}`)
  return {
    status: 'conflicts',
    message: 'Please resolve rebase conflicts before reviewing'
  }
}
